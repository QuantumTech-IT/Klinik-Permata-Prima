                    ...
                    if(!tbDokter.getValueAt(i,0).toString().equals("")){
                        try {
                            if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>=0){
                                double realInput = Valid.SetAngka(tbDokter.getValueAt(i,0).toString());
                                String kodeBrg   = tbDokter.getValueAt(i,1).toString();

                                    // konversi ke stok dasar (BOX)
                                    double stokDasar = getStokDasar(kodeBrg, realInput);
                                    if(Sequel.menyimpantf2("tokoopname","?,?,?,?,?,?,?,?","Stok Opname",8,new String[]{
                                            ...
                                            tbDokter.getValueAt(i,0).toString(), // simpan real input untuk opname
                                            tbDokter.getValueAt(i,7).toString(),
                                            tbDokter.getValueAt(i,8).toString(),
                                            catatan.getText()
                                        })==true){

                                        Trackbarang.catatRiwayat(kodeBrg, realInput, 0,"Opname",akses.getkode(),"Simpan");

                                        // update stok di DB pakai stok dasar
                                        Sequel.mengedit3("tokobarang","kode_brng=?","stok=?",2,new String[]{
                                            String.valueOf(stokDasar), kodeBrg
                                        });
//                                if(Sequel.menyimpantf2("tokoopname","?,?,?,?,?,?,?,?","Stok Opname",8,new String[]{
...
try {
    ...
    pstampil.setString(1, key);
    pstampil.setString(2, key);
    ...
    while (rstampil.next()) {
        tabMode.addRow(new Object[]{
            "",                                 // Real (kosong, diisi user saat opname)
            rstampil.getString("kode_brng"),
            rstampil.getString("nama_brng"),
            rstampil.getString("nm_jenis"),
            rstampil.getString("kode_sat2"),
            rstampil.getDouble("harga"),        // ✅ harga retail
            rstampil.getDouble("stok_total"),   // ✅ stok = stok*isi*kapasitas
            0,                                  // Selisih
            0                                   // Nominal Hilang
        });
    }
