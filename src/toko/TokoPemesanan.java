...
public class TokoPemesanan extends javax.swing.JDialog {
    ...
    public TokoPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul={"Jml","Kode Barang","Nama Barang","Satuan","G","Harga(Rp)","Subtotal(Rp)","Disk(%)","Diskon(Rp)","Total","Dasar","Distributor","Grosir","Retail", "No. Batch"};
        tabMode=new DefaultTableModel(null,judul){
              @Override public boolean isCellEditable(int rowIndex, int colIndex){
                boolean a = false;
                if ((colIndex==0)||(colIndex==4)||(colIndex==5)||(colIndex==7)||(colIndex==8)||(colIndex==14)) {
                    a=true;
                }
                return a;
             }
                ...
                if(Sequel.menyimpantf2("tokopemesanan","?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Faktur",14,new String[]{
                    ...
                    for(i=0;i<jml;i++){  
                        ...
                        if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                            if(Sequel.menyimpantf2("toko_detail_pesan","?,?,?,?,?,?,?,?,?,?","Transaksi Penerimaan",10,new String[]{
                                ...
                                tbDokter.getValueAt(i,7).toString(),tbDokter.getValueAt(i,8).toString(),tbDokter.getValueAt(i,9).toString(),tbDokter.getValueAt(i,14).toString()
                            })==true){
