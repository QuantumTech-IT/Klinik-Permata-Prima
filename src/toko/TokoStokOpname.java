...
private void StokKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_StokKeyPressed
    ...
    */
    ...
    String sql =
        "SELECT " +
        "  tokoopname.kode_brng, " +                   // 1
        "  tokobarang.nama_brng, " +                   // 2
        "  tokoopname.dasar, " +                       // 3 (harga dasar opname)
        "  tokobarang.kode_sat2 AS sat_kecil, " +     // 4 Satuan terkecil
        "  tokoopname.tanggal, " +                     // 5
        "  tokoopname.stok, " +          // 6 Stok tampil = kapasitas
        "  tokoopname.real, " +                        // 7
        "  tokoopname.selisih, " +                     // 8
        "  (tokoopname.real * tokoopname.dasar) AS totalreal, " + // 9
        "  tokoopname.nomihilang, " +                  // 10
        "  tokoopname.keterangan " +                   // 11
        "FROM tokoopname " +
        "INNER JOIN tokobarang ON tokoopname.kode_brng = tokobarang.kode_brng " +
        "WHERE tokoopname.tanggal BETWEEN ? AND ? " +
        "  AND (tokoopname.kode_brng LIKE ? " +
        "       OR tokobarang.nama_brng LIKE ? " +
        "       OR tokobarang.kode_sat2 LIKE ? " +
        "       OR tokoopname.keterangan LIKE ?) " +
        "ORDER BY tokoopname.tanggal";

    try {
        pstampil = koneksi.prepareStatement(sql);
        try {
            // Isi parameter (6 parameter saja, lebih ringkas & aman)
            pstampil.setString(1, tglAwal);
            pstampil.setString(2, tglAkhir);
            pstampil.setString(3, key);
            pstampil.setString(4, key);
            pstampil.setString(5, key);
            pstampil.setString(6, key);

            rstampil = pstampil.executeQuery();

            while (rstampil.next()) {
                // akumulasi total
                totalreal += rstampil.getDouble("totalreal"); // kolom alias
                total     += rstampil.getDouble("nomihilang"); // kolom alias

                tabMode.addRow(new Object[]{
                    rstampil.getString(1),                        // Kode Barang
                    rstampil.getString(2),                        // Nama Barang
                    df2.format(rstampil.getDouble(3)),            // Harga Dasar
                    rstampil.getString("sat_kecil"),              // ✅ Satuan terkecil (kode_sat2)
                    rstampil.getString(5),                        // Tanggal
                    rstampil.getString(6),                        // ✅ Stok = kapasitas
                    rstampil.getString(7),                        // Real
                    rstampil.getString(8),                        // Selisih
                    df2.format(rstampil.getDouble("totalreal")),  // Total Real (uang)
                    df2.format(rstampil.getDouble("nomihilang")), // Nominal Hilang (Rp)
                    rstampil.getString(11)                        // Keterangan
                });
            }
        } catch (Exception e) {
            System.out.println("Notif Opname : " + e);
        } finally {
            if (rstampil != null) { try { rstampil.close(); } catch (Exception ig) {} }
            if (pstampil != null) { try { pstampil.close(); } catch (Exception ig) {} }
        }
    } catch (Exception e) {
