...
import fungsi.WarnaTableGroup;
import fungsi.WarnaTableNota;
...
public class TokoCariPenjualan extends javax.swing.JDialog {
    ...
    private final HashMap<String,String> mapNoRawat = new HashMap<>();
    ...
    public TokoCariPenjualan(java.awt.Frame parent, boolean modal) {
        ...
        //setUndecorated(true); 
        initComponents();
        btnRetur = new javax.swing.JButton("Retur Item");
btnRetur.addActionListener(e -> prosesRetur());
panelisi3.add(btnRetur);
        
        Object[] row={"Tanggal","No.Nota","Petugas","Member","Catatan","Jns.Jual","Cara Bayar", "Dokter Peresep"};
        ...
        //tbDokter.setDefaultRenderer(Object.class, new WarnaTableGroup(1));
       tbDokter.setDefaultRenderer(Object.class, new WarnaTableNota(1));
        tbDokter.setRowHeight(26);
        tbDokter.setRowMargin(2); // jarak antar row
        tbDokter.setShowGrid(true);
        tbDokter.setGridColor(new Color(235,235,235));
    ...
    private void initComponents() {
        ...
        btnLihatPenjualanResep = new javax.swing.JButton();
        ...
        btnLihatPenjualanResep.setText("Lihat Penjualan Resep");
        btnLihatPenjualanResep.setName("btnLihatPenjualanResep"); // NOI18N
        btnLihatPenjualanResep.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnLihatPenjualanResepActionPerformed(evt);
            }
        });
        panelisi1.add(btnLihatPenjualanResep);
        ...
        NoNota.setBounds(60, 10, 170, 23);

        label11.setText("Tanggal :");
        label11.setName("label11"); // NOI18N
        label11.setPreferredSize(new java.awt.Dimension(70, 23));
        panelisi3.add(label11);
        label11.setBounds(10, 40, 50, 23);
        ...
        label18.setBounds(150, 40, 30, 23);
        ...
        panelisi3.add(btnRetur);
        btnRetur.setBounds(300, 10, 80, 23);

        jButton1.setText("Aturan Pakai");
        ...
        panelisi3.add(jButton1);
        jButton1.setBounds(290, 40, 100, 23);
...
private void btnLihatPenjualanResepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLihatPenjualanResepActionPerformed
      String nota = getNotaTerpilihDariTbDokter();
    if (nota.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Pilih dulu nota yang mau dilihat!");
        return;
    }

    String noRawat = mapNoRawat.getOrDefault(nota, "").trim();
    if (noRawat.isEmpty() || noRawat.equals("-") || noRawat.equalsIgnoreCase("null")) {
        noRawat = Sequel.cariIsi("SELECT no_rawat FROM tokopenjualan WHERE nota_jual=?", nota);
    ...
    DlgResepTokoPerNoRawat dlg = new DlgResepTokoPerNoRawat(parent, true);
    dlg.setNoNota(nota);
    if (!(noRawat.isEmpty() || noRawat.equals("-") || noRawat.equalsIgnoreCase("null"))) {
        dlg.setNoRawat(noRawat);
    }
    dlg.tampil();
    dlg.setVisible(true);

    }//GEN-LAST:event_btnLihatPenjualanResepActionPerformed
    ...
    */
    ...
    private javax.swing.JButton btnLihatPenjualanResep;
    ...
    private void tampil() {
        ...
        try{
            ps = koneksi.prepareStatement(
                    "select distinct tokopenjualan.nota_jual, tokopenjualan.tgl_jual, " +
                    "tokopenjualan.nip,petugas.nama,tokopenjualan.ongkir,tokopenjualan.total, " +
                    "tokopenjualan.kd_dokter, dokter.nm_dokter, " +
                    ...
                    "tokopenjualan.no_rawat " +  // ✅ tambah ini
                    ...
                    "left join dokter  on dokter.kd_dokter=tokopenjualan.kd_dokter " +
                    ...
                    " tokojenisbarang.nm_jenis like '%"+TCari.getText()+"%' or " +
                    " tokopenjualan.total like '%"+TCari.getText()+"%') " +
            ...
            try {
                ...
                ttlongkir=0;
                while(rs.next()){      
                    String nota = rs.getString("nota_jual");
                    String norw = rs.getString("no_rawat");
                    mapNoRawat.put(nota, (norw == null ? "" : norw));
                    tabMode.addRow(new String[]{
                        rs.getString("tgl_jual"),rs.getString("nota_jual"),rs.getString("nip")+", "+rs.getString("nama"),
                        rs.getString("no_member")+" "+rs.getString("nm_member"),rs.getString("keterangan"),rs.getString("jns_jual"),rs.getString("nama_bayar"), rs.getString("nm_dokter")
                    });
                    tabMode.addRow(new String[]{
                        "Kode Barang","Nama Barang","Jml & Satuan & Harga(Rp)","Subtotal(Rp)","Ptg(%) Potongan(Rp)","Tuslah(Rp)","Total"
                    });
                            ...
                            "<td colspan='6'>"+
                                "<table width='100%' border='0' align='center' cellpadding='2px' cellspacing='0' class='tbl_form'>"+
                                    "<tr class='isi'>"+
                                        ...
                                        "<td valign='top' bgcolor='#fdfff9' align='center' width='10%'>Tuslah(Rp)</td>"+        
    ...
    if (r < 0) {
        javax.swing.JOptionPane.showMessageDialog(this, "Pilih satu nota dulu.");
        return;
    }
    int m = tbDokter.convertRowIndexToModel(r);
   String noNota = String.valueOf(tabMode.getValueAt(r, 1)); // kolom No.Nota
    DlgReturPenjualan dlg = new DlgReturPenjualan(null, true);
    dlg.setNoNota(noNota);
    dlg.setLocationRelativeTo(this);
    dlg.setVisible(true);
    // setelah dialog close: refresh laporan jika perlu
     tampil();
}
    
    
    public class DlgReturPenjualan extends javax.swing.JDialog {
    private javax.swing.JTable tbItem;
    private javax.swing.table.DefaultTableModel tabModel;

    // simpan nomor nota yang sedang diproses
    private String noNota;

    public DlgReturPenjualan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        // kalau mau borderless, pertahankan baris di bawah; jika tidak perlu, hapus saja.
        //setUndecorated(true);  // HARUS sebelum initComponents() kalau dipakai

        initComponents();      // kalau NetBeans generate komponen; aman untuk dibiarkan
        buildUI();             // rakit tabel & tombol ke content pane
    }
    ...
    private void buildUI() {
        tabModel = new javax.swing.table.DefaultTableModel(
            null,
            new Object[]{"Pilih", "Kode", "Nama Barang", "Jml Jual", "Jml Retur", "Harga", "Subtotal"}
        ) {
            @Override public Class<?> getColumnClass(int col) {
                // 0=checkbox, 3/4/5/6 numerik
                return (col == 0) ? Boolean.class
                     : (col == 3 || col == 4 || col == 5 || col == 6) ? Double.class
                     : String.class;
            }
            @Override public boolean isCellEditable(int r, int c) {
                return (c == 0 || c == 4); // hanya centang & jml retur yang boleh diedit
            }
        };

        tbItem = new javax.swing.JTable(tabModel);
        tbItem.setAutoCreateRowSorter(true);

        // tambahkan ke CONTENT PANE dialog
        getContentPane().setLayout(new java.awt.BorderLayout());
        getContentPane().add(new javax.swing.JScrollPane(tbItem), java.awt.BorderLayout.CENTER);

        javax.swing.JButton btnSimpan = new javax.swing.JButton("Simpan Retur");
        btnSimpan.addActionListener(e -> simpanRetur());
        getContentPane().add(btnSimpan, java.awt.BorderLayout.SOUTH);
    ...
    private void loadItemDariNota(String noNota) {
        // kolom sesuai yang kamu sebut: jumlah, kode_sat, h_jual, nota_jual
        final String sql =
            "SELECT d.kode_brng, b.nama_brng, d.jumlah, d.kode_sat, d.h_jual, (d.jumlah*d.h_jual) AS subtotal " +
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    tabModel.addRow(new Object[]{
                        Boolean.FALSE,          // Pilih
                        rs.getString(1),        // Kode
                        rs.getString(2),        // Nama Barang
                        rs.getDouble(3),        // Jml Jual
                        0.0,                    // Jml Retur (editable)
                        rs.getDouble(5),        // Harga
                        rs.getDouble(6)         // Subtotal
                    });
                }
            }
        } catch (Exception ex) {
            javax.swing.JOptionPane.showMessageDialog(this, "Gagal load item: " + ex.getMessage());
        }
    }
    ...
    private void simpanRetur() {
        if (getNoNota() == null || getNoNota().trim().isEmpty()) {
            javax.swing.JOptionPane.showMessageDialog(this, "Nomor nota kosong.");
            return;
        }

        String noRetur = buatNomorReturOtomatis();
        ...
        try (PreparedStatement ps = koneksi.prepareStatement(insertSql)) {
            for (int i = 0; i < tabModel.getRowCount(); i++) {
                Boolean pilih = (Boolean) tabModel.getValueAt(i, 0);
                Double jmlRetur = (Double) tabModel.getValueAt(i, 4);
                if (Boolean.TRUE.equals(pilih) && jmlRetur != null && jmlRetur > 0) {
                    String kode = String.valueOf(tabModel.getValueAt(i, 1));
                    Double harga = (Double) tabModel.getValueAt(i, 5);

                    ps.setString(1, noRetur);
                    ps.setString(2, getNoNota());
                    ps.setString(3, kode);
                    ps.setDouble(4, jmlRetur);
                    ps.setString(5, "PCS");               // bisa diambil dari d.kode_sat bila mau
                    ps.setDouble(6, harga);
                    ps.setDouble(7, jmlRetur * harga);
                    ps.setDate(8, tglRetur);
                    ps.setString(9, akses.getkode());     // NIP petugas (dari session)
                    ps.addBatch();
                    inserted++;
                }
            }
            ...
            if (inserted > 0) {
                ps.executeBatch();
                javax.swing.JOptionPane.showMessageDialog(this, "Retur berhasil disimpan! No.Retur: " + noRetur);
                dispose();
            } else {
                javax.swing.JOptionPane.showMessageDialog(this, "Tidak ada item yang dipilih untuk retur.");
            }

        } catch (Exception ex) {
            javax.swing.JOptionPane.showMessageDialog(this, "Gagal simpan retur: " + ex.getMessage());
        }
    }

    private String buatNomorReturOtomatis() {
        return "RT" + new java.text.SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date());
    }
}
   public void setNoNota(String noNota) {
    this.noNota = noNota;
   ...
   // di bagian bawah kelas TokoCariPenjualan.java
private void onKlikRetur() {
    int r = tbDokter.getSelectedRow();
    if (r < 0) {
        JOptionPane.showMessageDialog(this, "Pilih satu nota dulu.");
        return;
    }
    int m = tbDokter.convertRowIndexToModel(r);
    String noNota = String.valueOf(tabMode.getValueAt(m, 1)); // kolom No.Nota

    java.awt.Window w = javax.swing.SwingUtilities.getWindowAncestor(this);
    java.awt.Frame f = (w instanceof java.awt.Frame) ? (java.awt.Frame) w : null;

    DlgReturPenjualanToko dlg =
        new DlgReturPenjualanToko(f, true, koneksi, akses.getkode());
    dlg.setNoNota(noNota);
    dlg.setVisible(true);

    // refresh laporan bila perlu
    // tampilLaporan();
}

private void onKlikCetakEtiket() {
    int r = tbDokter.getSelectedRow();
    if (r < 0) {
        JOptionPane.showMessageDialog(this, "Pilih satu nota dulu.");
        return;
    ...
    // ambil no_resep dari header resep_toko yang sudah dilink ke penjualan
    String noResep = Sequel.cariIsi("SELECT no_resep FROM resep_toko WHERE no_nota=?", noNota);
    if (noResep == null || noResep.trim().isEmpty()) {
        JOptionPane.showMessageDialog(this, "Tidak ditemukan No. Resep untuk nota: " + noNota);
        return;
    }

    // parameter rumah sakit
    Map<String, Object> param = new HashMap<>();
    param.put("namars", akses.getnamars());
    param.put("alamatrs", akses.getalamatrs());
    param.put("kotars", akses.getkabupatenrs());
    ...
    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    try {
        // ===== cetak NON RACIKAN ke rptItemResep.jasper =====
        int cntNR = Sequel.cariInteger(
            "SELECT COUNT(*) FROM resep_toko_detail WHERE no_resep=? AND keterangan='Non Racikan'",
            noResep
        );
        if (cntNR > 0) {
            Valid.MyReportqry(
                "rptItemResep.jasper",
                "report",
                "::[ Aturan Pakai Obat (Non Racikan) ]::",
                // alias-kan field agar cocok dengan report lama
                "SELECT " +
                "  rt.no_resep                                      AS no_resep, " +
                "  DATE(rt.tgl_resep)                               AS tgl_perawatan, " +
                "  TIME(rt.tgl_resep)                               AS jam, " +
                "  ps.tgl_lahir                                     AS tgl_lahir, " +
                "  rt.no_rawat                                      AS no_rawat, " +
                "  ps.no_rkm_medis                                  AS no_rkm_medis, " +
                "  ps.nm_pasien                                     AS nm_pasien, " +
                "  tb.nama_brng                                     AS nama_brng, " +
                "  rtd.aturan_pakai                                 AS aturan, " +
                "  rtd.jml                                          AS jml, " +
                "  rtd.satuan                                       AS satuan, " +
                "  ps.jk                                            AS jk, " +
                "  rp.umurdaftar                                    AS umurdaftar, " +
                "  rp.sttsumur                                      AS sttsumur " +
                "FROM resep_toko rt " +
                "LEFT JOIN resep_toko_detail rtd ON rtd.no_resep = rt.no_resep " +
                "LEFT JOIN tokobarang tb          ON tb.kode_brng = rtd.kode_brng " +
                "LEFT JOIN reg_periksa rp         ON rp.no_rawat = rt.no_rawat " +
                "LEFT JOIN pasien ps              ON ps.no_rkm_medis = rp.no_rkm_medis " +
                "WHERE rt.no_resep = '" + noResep + "' AND rtd.keterangan = 'Non Racikan'",
                param
            );
        }

        // ===== cetak RACIKAN ke rptItemResep2.jasper =====
        int cntR = Sequel.cariInteger(
            "SELECT COUNT(*) FROM resep_toko_detail WHERE no_resep=? AND keterangan='Racikan'",
            noResep
        );
        if (cntR > 0) {
            Valid.MyReportqry(
                "rptItemResep2.jasper",
                "report",
                "::[ Aturan Pakai Obat (Racikan) ]::",
                // sesuaikan alias field yang diminta report racikan
                "SELECT " +
                "  rt.no_resep                                      AS no_resep, " +
                "  DATE(rt.tgl_resep)                               AS tgl_perawatan, " +
                "  TIME(rt.tgl_resep)                               AS jam, " +
                ...
                "  ps.no_rkm_medis                                  AS no_rkm_medis, " +
                "  ps.nm_pasien                                     AS nm_pasien, " +
                "  rtd.nama_racikan                                 AS nama_racik, " +
                "  rtd.aturan_pakai                                 AS aturan_pakai, " +
                "  rtd.jml                                          AS jml_dr, " +   // report lama pakai 'jml_dr'
                "  'Racik'                                          AS nm_racik, " + // placeholder metode racik
                "  ps.jk                                            AS jk, " +
                "  rp.umurdaftar                                    AS umurdaftar, " +
                "  rp.sttsumur                                      AS sttsumur " +
                "FROM resep_toko rt " +
                ...
                "LEFT JOIN reg_periksa rp         ON rp.no_rawat = rt.no_rawat " +
                "LEFT JOIN pasien ps              ON ps.no_rkm_medis = rp.no_rkm_medis " +
                "WHERE rt.no_resep = '" + noResep + "' AND rtd.keterangan = 'Racikan' " +
                "GROUP BY rt.no_resep, rtd.nama_racikan",
                param
            );
        }

        if (cntNR == 0 && cntR == 0) {
            JOptionPane.showMessageDialog(this, "Tidak ada item untuk dicetak pada resep: " + noResep);
        }
    } finally {
        this.setCursor(Cursor.getDefaultCursor());
    }
...
private String getNotaTerpilihDariTbDokter() {
    int vRow = tbDokter.getSelectedRow();
    if (vRow == -1) return "";

    int mRow = tbDokter.convertRowIndexToModel(vRow);

    // scan ke atas sampai ketemu baris header nota
    for (int r = mRow; r >= 0; r--) {
        Object v = tabMode.getValueAt(r, 1); // kolom No.Nota
        if (v != null) {
            String s = v.toString().trim();
            if (s.isEmpty()) continue;
            if (mapNoRawat.containsKey(s)) return s;
            if (s.matches("^[A-Za-z]{1,4}\\d{6,}$")) return s;
        }
    }

    Object fallback = tabMode.getValueAt(mRow, 1);
    if (fallback != null) {
        String s = fallback.toString().trim();
