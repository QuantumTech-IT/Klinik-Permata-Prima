...
public class TokoCariPemesanan extends javax.swing.JDialog {
    ...
    public TokoCariPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"No.Faktur","Suplier","Petugas","Jml","Harga(Rp)","SubTotal(Rp)",
                    "Disk(%)","Bsr.Disk(Rp)","Total(Rp)", "No. Batch"};
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    ...
    */
    ...
    try {
        // === HEADER / FAKTUR ===
        String sqlHeader =
            "SELECT p.tgl_pesan, p.no_faktur, p.kode_suplier, s.nama_suplier, " +
            "       p.nip, pt.nama, p.tgl_faktur, p.tgl_tempo, p.status, " +
            "       p.total2, p.ppn, p.meterai, p.tagihan, p.no_order " +
            "FROM tokopemesanan p " +
            "JOIN tokosuplier s ON p.kode_suplier=s.kode_suplier " +
            "JOIN petugas pt     ON p.nip=pt.nip " +
            // ikutkan detail & master utk filter & grouping
            "JOIN toko_detail_pesan dp ON p.no_faktur=dp.no_faktur " +
            "JOIN tokobarang b   ON dp.kode_brng=b.kode_brng " +
            "JOIN kodesatuan ks  ON dp.kode_sat=ks.kode_sat " +
            "JOIN tokojenisbarang jb ON b.jenis=jb.kd_jenis " +
            "WHERE (p.tgl_pesan BETWEEN ? AND ?) " +
            "  AND p.no_faktur LIKE ? " +
            "  AND s.nama_suplier LIKE ? " +
            "  AND pt.nama LIKE ? " +
            "  AND jb.nm_jenis LIKE ? " +
            "  AND b.nama_brng LIKE ? " +
            // PENCARIAN UMUM (TCari) termasuk NO_BATCH
            "  AND ( p.no_faktur LIKE ? " +
            "     OR p.kode_suplier LIKE ? " +
            "     OR p.nip LIKE ? " +
            "     OR pt.nama LIKE ? " +
            "     OR dp.kode_brng LIKE ? " +
            "     OR b.nama_brng LIKE ? " +
            "     OR dp.kode_sat LIKE ? " +
            "     OR p.no_order LIKE ? " +
            "     OR jb.nm_jenis LIKE ? " +
            ...
            "ORDER BY p.tgl_pesan, p.no_faktur";

        ps = koneksi.prepareStatement(sqlHeader);
        ...
        // TCari dipakai ke banyak kolom, termasuk no_batch
        String cari = "%"+TCari.getText()+"%";
        ps.setString(8,  cari); // p.no_faktur
        ps.setString(9,  cari); // p.kode_suplier
        ps.setString(10, cari); // p.nip
        ps.setString(11, cari); // pt.nama
        ps.setString(12, cari); // dp.kode_brng
        ps.setString(13, cari); // b.nama_brng
        ps.setString(14, cari); // dp.kode_sat
        ps.setString(15, cari); // p.no_order
        ps.setString(16, cari); // jb.nm_jenis
        ps.setString(17, cari); // dp.no_batch

        rs = ps.executeQuery();
        tagihan = 0;

        // === STATEMENT tambahan: ambil daftar batch per faktur (untuk tampil di bawah faktur) ===
        PreparedStatement psBatch = koneksi.prepareStatement(
            "SELECT GROUP_CONCAT(DISTINCT dp.no_batch ORDER BY dp.no_batch SEPARATOR ', ') AS batches " +
            "FROM toko_detail_pesan dp WHERE dp.no_faktur=?"
        );

        // === DETAIL per faktur ===
        String sqlDetail =
            "SELECT dp.kode_brng, b.nama_brng, dp.kode_sat, ks.satuan, " +
            "       dp.jumlah, dp.harga, dp.no_batch, dp.subtotal, dp.dis, dp.besardis, dp.total " +
            "FROM toko_detail_pesan dp " +
            "JOIN tokobarang b   ON dp.kode_brng=b.kode_brng " +
            "JOIN kodesatuan ks  ON dp.kode_sat=ks.kode_sat " +
            "JOIN tokojenisbarang jb ON b.jenis=jb.kd_jenis " +
            "WHERE dp.no_faktur=? " +
            "  AND jb.nm_jenis LIKE ? " +
            "  AND b.nama_brng LIKE ? " +
            "  AND ( dp.kode_brng LIKE ? " +
            "     OR dp.no_batch LIKE ? " +     // ← masuk pencarian
            "     OR b.nama_brng LIKE ? " +
            "     OR dp.kode_sat LIKE ? " +
            "     OR jb.nm_jenis LIKE ? ) " +
            "ORDER BY dp.kode_brng";
        ...
        while (rs.next()) {
            ...
            // BARIS NO. BATCH (tepat di bawah no_faktur)
            String daftarBatch = "-";
            try {
                psBatch.setString(1, rs.getString("no_faktur"));
                try (ResultSet rb = psBatch.executeQuery()) {
                    if (rb.next() && rb.getString(1) != null && !rb.getString(1).isEmpty()) {
                        daftarBatch = rb.getString(1);
                    }
                }
            } catch (Exception ignore) {}
            tabMode.addRow(new Object[]{"", "No. Batch", ": " + daftarBatch, "", "", "", "", "", ""});

            // DETAIL ITEM
            ps2 = koneksi.prepareStatement(sqlDetail);
            ps2.setString(1, rs.getString("no_faktur"));
            ps2.setString(2, "%"+nmjenis.getText()+"%");
            ps2.setString(3, "%"+nmbar.getText()+"%");
            ps2.setString(4, cari); // kode_brng
            ps2.setString(5, cari); // no_batch
            ps2.setString(6, cari); // nama_brng
            ps2.setString(7, cari); // kode_sat
            ps2.setString(8, cari); // nm_jenis

            rs2 = ps2.executeQuery();
            int no = 1;
            while (rs2.next()) {
                // mapping kolom: subtotal/total tetap benar, no_batch tidak menggeser
                tabMode.addRow(new Object[]{
                    no + ". " + rs2.getString(1),                 // kode_brng
                    rs2.getString(2),                             // nama_brng
                    rs2.getString(3) + ", " + rs2.getString(4),   // kode_sat, satuan
                    rs2.getString(5),                             // jumlah
                    Valid.SetAngka(rs2.getDouble(6)),             // harga
                    Valid.SetAngka(rs2.getDouble(8)),             // subtotal
                    Valid.SetAngka(rs2.getDouble(9)),             // dis
                    Valid.SetAngka(rs2.getDouble(10)),            // besardis
                    Valid.SetAngka(rs2.getDouble(11))             // total
                });
                // opsional: baris mini di bawah item untuk batch per item (aktifkan jika mau)
                // tabMode.addRow(new Object[]{"", "  Batch", ": "+rs2.getString(7), "", "", "", "", "", ""});
                no++;
            }
            if (rs2 != null) rs2.close();
            if (ps2 != null) ps2.close();

            // FOOTER / RINGKASAN FAKTUR
