...
import fungsi.BiometrikDao;
import fungsi.FingerprintService;
import fungsi.DummyFingerprintService;
...
public final class SuratPersetujuanPenolakanTindakan extends javax.swing.JDialog {
    ...
    private final BiometrikDao biomDao = new BiometrikDao(koneksiDB.condb());
    private final FingerprintService fpSvc = new DummyFingerprintService();
    ...
    private void initComponents() {
        ...
        TglPernyataan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-10-2025" }));
        ...
        TglLahirPenerima.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-10-2025" }));
        ...
        DTPCari1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-10-2025" }));
        ...
        DTPCari2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-10-2025" }));
...
int row = tbObat.getSelectedRow();
    ...
    String noPernyataan = String.valueOf(tbObat.getValueAt(row, 0)).trim();
    String noRawat      = String.valueOf(tbObat.getValueAt(row, 1)).trim();
    String nama = Sequel.cariIsi(
        "SELECT p.nm_pasien FROM reg_periksa r INNER JOIN pasien p ON p.no_rkm_medis=r.no_rkm_medis WHERE r.no_rawat=?", noRawat);
    String nik  = Sequel.cariIsi(
        "SELECT p.no_ktp FROM reg_periksa r INNER JOIN pasien p ON p.no_rkm_medis=r.no_rkm_medis WHERE r.no_rawat=?", noRawat);

    try {
        // 1) capture fingerprint
        byte[] tpl     = fpSvc.captureProbeTemplate();   // bisa lempar Exception
        String deviceSn = fpSvc.getDeviceSerial();

        // 2) (opsional) transaksi proses lama + simpan biometrik
        Sequel.AutoComitFalse();

        Sequel.queryu("DELETE FROM antripersetujuan");
        Sequel.queryu("INSERT INTO antripersetujuan VALUES('" + noPernyataan + "','" + noRawat + "')");
        Sequel.queryu("DELETE FROM bukti_persetujuan_penolakan_tindakan_penerimainformasi WHERE no_pernyataan='" + noPernyataan + "'");
        Sequel.queryu("DELETE FROM bukti_persetujuan_penolakan_tindakan_saksikeluarga WHERE no_pernyataan='" + noPernyataan + "'");

        biomDao.upsertBukti(                      // bisa lempar Exception
            noPernyataan, "pasien", nik, nama,
            1, tpl, null, null, deviceSn
        );

        Sequel.Commit();
        JOptionPane.showMessageDialog(this, "Berhasil diambil & fingerprint tersimpan.");
    } catch (Exception ex) {
        Sequel.RollBack();
        JOptionPane.showMessageDialog(this, "Operasi dibatalkan/gagal: " + ex.getMessage());
    } finally {
