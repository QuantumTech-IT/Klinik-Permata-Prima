...
//                          "detail_pemberian_obat.no_rawat=? group by detail_pemberian_obat.kode_brng order by jenis.nama",
            // == Cari Obat dari modul Toko (masuk ke Obat & BHP Billing) ==
//String sqlpscaritoko =
//    "SELECT tb.nama_brng, j.nama, " +
//    "       td.h_jual AS biaya_obat, " +
//    "       SUM(td.jml_jual) AS jml, " +
//    "       SUM(COALESCE(td.embalase,0)+COALESCE(td.tuslah,0)) AS tambahan, " +
//    "       (SUM(td.jml_jual*td.h_jual) - SUM(COALESCE(td.embalase,0)+COALESCE(td.tuslah,0))) AS total, " +
//    "       SUM(COALESCE(td.h_beli, tb.h_beli)*td.jml_jual) AS totalbeli " +  // fallback pakai hbeli di master
//    "FROM tokopenjualan tp " +
//    "JOIN tokodetailjual td ON td.no_nota = tp.no_nota " +
//    "JOIN tokobarang tb     ON tb.kode_brng = td.kode_brng " +
//    "JOIN jenis j           ON j.kdjns = tb.kdjns " +
//    "WHERE tp.no_rawat = ? " +
//    "GROUP BY td.kode_brng " +
//    "ORDER BY j.nama";,
            ...
            if(!tbBilling.getValueAt(0,1).toString().trim().contains("No.Nota")){
                ...
                try{
                    ...
                    while(rsakunbayar.next()){
                       Sequel.menyimpan("tampjurnal","'"+rsakunbayar.getString("kd_rek")+"','"+rsakunbayar.getString("nama_bayar")+"','0','"+rsakunbayar.getString("besar_bayar")+"'","kredit=kredit+'"+(rsakunbayar.getString("besar_bayar"))+"'","kd_rek='"+rsakunbayar.getString("kd_rek")+"'");
                    } 
                }catch (Exception e) {
                    sukses=false;
                ...
                try{
                    ...
                    while(rsakunpiutang.next()){                    
                        Sequel.menyimpan("tampjurnal","'"+rsakunpiutang.getString("kd_rek")+"','"+rsakunpiutang.getString("nama_bayar")+"','0','"+rsakunpiutang.getString("totalpiutang")+"'","kredit=kredit+'"+(rsakunpiutang.getString("totalpiutang"))+"'","kd_rek='"+rsakunpiutang.getString("kd_rek")+"'");
                    } 
                }catch (Exception e) {
                    sukses=false;
    ...
    */
    ...
    public void isRawat2() {
        ...
        if(chkObat.isSelected()==true){
            tabModeRwJlDr.addRow(new Object[]{true,"Obat & BHP",":","",null,null,null,null,"Obat"});
            prosesCariObat(); 
//            String noRM = TNoRM.getText().trim(); // pastikan field ini ada di form
//    java.sql.Timestamp mulai = getAwalEpisode(TNoRw.getText());
//    java.sql.Timestamp sampai = getAkhirEpisodeEksklusif(TNoRw.getText());
//
//    prosesCariPenjualanTokoPerRM(noRM, mulai, sampai);
    ...
    // --- Tambahan variabel di kelas (opsional) ---
private double totalToko = 0d;
private PreparedStatement psTokoJual = null, psTokoRetur = null;
private ResultSet rsTokoJual = null, rsTokoRetur = null;

// --- SQL versi PENGHUBUNG no_rawat <-> no_nota (disarankan) ---
private static final String SQL_TOTAL_JUAL_TOKO_LINK =
    "SELECT COALESCE(SUM(d.subtotal),0) AS total_jual " +
    "FROM tokopenjualan v " +
    "JOIN tokopenjualan j ON j.nota_jual = v.nota_jual " +
    "JOIN toko_detail_jual d ON d.nota_jual = j.nota_jual " +
    "WHERE v.no_member = ?";

private static final String SQL_TOTAL_RETUR_TOKO_LINK =
    "SELECT COALESCE(SUM(r.subtotal),0) AS total_retur " +
    "FROM tokopenjualan v " +
    "JOIN toko_retur_penjualan r ON r.no_nota = v.nota_jual " +
    "WHERE v.no_member = ?";

// --- SQL versi fallback (tanpa tabel penghubung) ---
// Jika di tabel penjualan sudah ada kolom no_rawat
private static final String SQL_TOTAL_JUAL_TOKO_FALLBACK =
    "SELECT COALESCE(SUM(d.subtotal),0) AS total_jual " +
    "FROM tokopenjualan j " +
    "JOIN toko_detail_jual d ON d.nota_jual = j.nota_jual " +
    "WHERE j.no_member = ?";

private static final String SQL_TOTAL_RETUR_TOKO_FALLBACK =
    "SELECT COALESCE(SUM(r.subtotal),0) AS total_retur " +
    "FROM toko_retur_penjualan r " +
    "JOIN tokopenjualan j ON j.nota_jual = r.no_nota " +
    "WHERE j.no_member = ?";

// --- Metode dipanggil di alur billing bersamaan dengan prosesCariObat() ---
private void prosesCariPenjualanToko() {
    double totalJual = 0d;
    double totalRetur = 0d;
    totalToko = 0d;

    final String noRawat = TNoRw.getText() != null ? TNoRw.getText().trim() : "";
    if (noRawat.isEmpty()) return;

    try {
        // ==== PILIH SALAH SATU BLOK SQL BERDASARKAN SKEMA ====
        // Jika PUNYA tabel penghubung no_rawat <-> no_nota (DISARANKAN):
        psTokoJual = koneksi.prepareStatement(SQL_TOTAL_JUAL_TOKO_LINK);
        psTokoRetur = koneksi.prepareStatement(SQL_TOTAL_RETUR_TOKO_LINK);

        // // Jika TIDAK punya penghubung dan kolom no_rawat ada di penjualan, gunakan fallback:
        // psTokoJual = koneksi.prepareStatement(SQL_TOTAL_JUAL_TOKO_FALLBACK);
        // psTokoRetur = koneksi.prepareStatement(SQL_TOTAL_RETUR_TOKO_FALLBACK);

        // --- isi parameter & eksekusi total penjualan ---
        psTokoJual.setString(1, noRawat);
        rsTokoJual = psTokoJual.executeQuery();
        if (rsTokoJual.next()) {
            totalJual = rsTokoJual.getDouble("total_jual");
        }

        // --- isi parameter & eksekusi total retur ---
        psTokoRetur.setString(1, noRawat);
        rsTokoRetur = psTokoRetur.executeQuery();
        if (rsTokoRetur.next()) {
            totalRetur = rsTokoRetur.getDouble("total_retur");
        }

        // --- net total toko ---
        totalToko = Math.max(0d, totalJual - totalRetur);

        // --- jika ada nilai, tambahkan baris ke tabel billing ---
        if (totalToko > 0d) {
            tabModeRwJlDr.addRow(new Object[]{
                true,                 // col 0: centang
                "",                   // col 1: kosong
                "Penjualan Toko",     // col 2: label kategori
                ":",                  // col 3: pemisah tampilan
                totalToko,            // col 4: nominal
                1,                    // col 5: qty (display)
                0,                    // col 6: diskon (jika ada logika diskon, isi di sini)
                totalToko,            // col 7: total akhir
                "Toko"                // col 8: tipe/kategori
            });
            // akumulasi ke subtotal global
            subttl += totalToko;
        }
        ...
        try { if (rsTokoJual != null) rsTokoJual.close(); } catch (Exception ignored) {}
        try { if (rsTokoRetur != null) rsTokoRetur.close(); } catch (Exception ignored) {}
        try { if (psTokoJual != null) psTokoJual.close(); } catch (Exception ignored) {}
        try { if (psTokoRetur != null) psTokoRetur.close(); } catch (Exception ignored) {}
    }
}
    ...
    private PreparedStatement psTokoJualRM, psTokoReturRM;
private ResultSet rsTokoJualRM, rsTokoReturRM;
private double totalTokoRM = 0d;

private static final String SQL_TOKO_JUAL_PER_RM =
    "SELECT COALESCE(SUM(d.subtotal),0) AS total_jual " +
    "FROM toko_penjualan j " +
    "JOIN toko_detail_jual d ON d.nota_jual = j.nota_jual " +
    "WHERE j.no_rkm_medis = ? " +
    "AND j.tgl_jual >= ? AND j.tgl_jual < ?";

private static final String SQL_TOKO_RETUR_PER_RM =
    "SELECT COALESCE(SUM(r.subtotal),0) AS total_retur " +
    "FROM toko_retur_penjualan r " +
    "JOIN toko_penjualan j ON j.nota_jual = r.no_nota " +
    "WHERE j.no_rkm_medis = ? " +
    "AND r.tgl_retur >= ? AND r.tgl_retur < ?";

private void prosesCariPenjualanTokoPerRM(String noRM, java.sql.Timestamp mulai, java.sql.Timestamp sampaiEksklusif) {
    if (noRM == null || noRM.trim().isEmpty() || mulai == null || sampaiEksklusif == null) return;

    double totalJual = 0d, totalRetur = 0d;
    totalTokoRM = 0d;

    try {
        psTokoJualRM = koneksi.prepareStatement(SQL_TOKO_JUAL_PER_RM);
        psTokoJualRM.setString(1, noRM.trim());
        psTokoJualRM.setTimestamp(2, mulai);
        psTokoJualRM.setTimestamp(3, sampaiEksklusif);
        rsTokoJualRM = psTokoJualRM.executeQuery();
        if (rsTokoJualRM.next()) totalJual = rsTokoJualRM.getDouble("total_jual");

        psTokoReturRM = koneksi.prepareStatement(SQL_TOKO_RETUR_PER_RM);
        psTokoReturRM.setString(1, noRM.trim());
        psTokoReturRM.setTimestamp(2, mulai);
        psTokoReturRM.setTimestamp(3, sampaiEksklusif);
        rsTokoReturRM = psTokoReturRM.executeQuery();
        if (rsTokoReturRM.next()) totalRetur = rsTokoReturRM.getDouble("total_retur");

        totalTokoRM = Math.max(0d, totalJual - totalRetur);

        if (totalTokoRM > 0d) {
            tabModeRwJlDr.addRow(new Object[]{
                true,                 // centang
                "",                   // kolom kosong
                "Penjualan Toko",     // label
                ":",                  // pemisah
                totalTokoRM,          // nominal
                1,                    // qty tampil
                0,                    // diskon jika ada
        ...
        System.out.println("Notifikasi (Toko per RM): " + e);
    } finally {
        try { if (rsTokoJualRM != null) rsTokoJualRM.close(); } catch (Exception ignore) {}
        try { if (rsTokoReturRM != null) rsTokoReturRM.close(); } catch (Exception ignore) {}
        try { if (psTokoJualRM != null) psTokoJualRM.close(); } catch (Exception ignore) {}
        try { if (psTokoReturRM != null) psTokoReturRM.close(); } catch (Exception ignore) {}
    }
}
                        ...
                        if(Valid.SetAngka(tbAkunBayar.getValueAt(r,2).toString())>0){
                            ...
                            if(countbayar>1){
                                if(Sequel.menyimpantf2("detail_nota_jalan","?,?,?,?","Akun bayar",4,new String[]{
                                        ...
                                        Sequel.menyimpan("tampjurnal","'"+tbAkunBayar.getValueAt(r,1).toString()+"','"+tbAkunBayar.getValueAt(r,0).toString()+"','"+Double.toString(itembayar)+"','0'","debet=debet+'"+Double.toString(itembayar)+"'","kd_rek='"+tbAkunBayar.getValueAt(r,1).toString()+"'");
                                ...
                                if(piutang<=0){
                                    if(Sequel.menyimpantf2("detail_nota_jalan","?,?,?,?","Akun bayar",4,new String[]{
                                            ...
                                            Sequel.menyimpan("tampjurnal","'"+tbAkunBayar.getValueAt(r,1).toString()+"','"+tbAkunBayar.getValueAt(r,0).toString()+"','"+Double.toString(total)+"','0'","debet=debet+'"+Double.toString(total)+"'","kd_rek='"+tbAkunBayar.getValueAt(r,1).toString()+"'");
                                            if(Host_to_Host_Bank_Jateng.equals(tbAkunBayar.getValueAt(r,1).toString())){
                                                if(Sequel.menyimpantf2("tagihan_bpd_jateng","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,''",16,new String[]{
                    ...
                    for(r=0;r<row2;r++){ 
                        if(!tabModeAkunPiutang.getValueAt(r,3).toString().equals("")){
                            ...
                            if(Sequel.menyimpantf2("detail_piutang_pasien","?,?,?,?,?,?","Aku Piutang",6,new String[]{
                                    ...
                                    Sequel.menyimpan("tampjurnal","'"+tabModeAkunPiutang.getValueAt(r,1).toString()+"','"+tabModeAkunPiutang.getValueAt(r,0).toString()+"','"+Double.toString(itempiutang)+"','0'","debet=debet+'"+Double.toString(itempiutang)+"'","kd_rek='"+tabModeAkunPiutang.getValueAt(r,1).toString()+"'");                 
                            }else{
                                sukses=false;
                            }
                        }             
   ...
   private java.sql.Timestamp getAwalEpisode(String noRawat) {
    java.sql.Timestamp hasil = null;
    try {
        // ambil tgl_masuk dari reg_periksa
        PreparedStatement ps = koneksi.prepareStatement(
            "SELECT CONCAT(tgl_registrasi,' ',jam_reg) AS awal " +
            "FROM reg_periksa WHERE no_rawat=? LIMIT 1"
        );
        ps.setString(1, noRawat);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            hasil = java.sql.Timestamp.valueOf(rs.getString("awal"));
        }
        rs.close();
        ps.close();
    } catch (Exception e) {
        System.out.println("Notifikasi (getAwalEpisode): " + e);
    }
    return hasil;
}

private java.sql.Timestamp getAkhirEpisodeEksklusif(String noRawat) {
    java.sql.Timestamp hasil = null;
    try {
        // ambil tgl_keluar, kalau null berarti pasien masih aktif
        PreparedStatement ps = koneksi.prepareStatement(
            "SELECT IF(tgl_keluar='0000-00-00' OR tgl_keluar IS NULL, " +
            "DATE_ADD(CONCAT(tgl_registrasi,' ',jam_reg), INTERVAL 1 DAY), " +
            "DATE_ADD(CONCAT(tgl_keluar,' ',jam_keluar), INTERVAL 1 DAY)) AS akhir " +
            "FROM reg_periksa WHERE no_rawat=? LIMIT 1"
        );
        ps.setString(1, noRawat);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            hasil = java.sql.Timestamp.valueOf(rs.getString("akhir"));
        }
        rs.close();
        ps.close();
    } catch (Exception e) {
        System.out.println("Notifikasi (getAkhirEpisodeEksklusif): " + e);
    }
    return hasil;
} 
