...
//        setSize(885,674); 
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        // Ambil posisi dan ukuran dari panelGlass9
        Rectangle captureArea = PanelWall.getBounds();
        Point panelLocation = PanelWall.getLocationOnScreen(); // Ambil posisi di layar

        // Pastikan area tangkap berada di lokasi yang benar
        Rectangle capture = new Rectangle(panelLocation.x, panelLocation.y, captureArea.width, captureArea.height);
...
//        System.out.println(index);
    ...
    private void BtnHapus1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapus1ActionPerformed
       String photo = null;
try {
    PreparedStatement ps = koneksi.prepareStatement("SELECT photo FROM hasil_pemeriksaan_usg_gambar WHERE no_rawat=?");
    ps.setString(1, TNoRawat.getText()); // Ambil no_rawat dari tabel
    ResultSet rs = ps.executeQuery();
    
    if (rs.next()) {
        photo = rs.getString("photo"); // Simpan nama file gambar
    }

    rs.close();
    ps.close();
} catch (Exception e) {
    System.out.println("Error: " + e.getMessage()); // Debugging jika ada error
}

// Cek apakah `photo` valid sebelum digunakan
if (photo != null && !photo.equals("") && !photo.equals("-")) {
    String imageUrl = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" +
                      koneksiDB.HYBRIDWEB() + "/hasilpemeriksaanusg/" + photo;

    System.out.println("URL Gambar: " + imageUrl); // Debugging untuk cek URL

    // Panggil fungsi untuk menampilkan gambar
    imageAssesment(imageUrl);
} else {
    System.out.println("Gambar tidak tersedia.");
}

//        imageAssesment("http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/imagefreehand/masterimage/obgyn.png");
