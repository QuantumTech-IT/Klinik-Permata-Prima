...
public final class RMHasilPemeriksaanUSG extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-03-2025 00:00:56" }));
        ...
        BtnEdit2.setBounds(10, 370, 180, 30);

        PanelWall.setBackground(new java.awt.Color(29, 29, 29));
        PanelWall.setBackgroundImage(new javax.swing.ImageIcon(getClass().getResource("/picture/obgyn.png"))); // NOI18N
        ...
        PanelWall.setBounds(60, 420, 670, 250);
         ...
         if (tbObat.getSelectedRow() != -1) { 
                  try {
                      ps = koneksi.prepareStatement(
                          "SELECT pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_tlp, hasil_pemeriksaan_usg_gambar.photo "
                          + "FROM hasil_pemeriksaan_usg_gambar "
                          + "INNER JOIN reg_periksa ON hasil_pemeriksaan_usg_gambar.no_rawat = reg_periksa.no_rawat "
                          + "INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                          + "WHERE reg_periksa.no_rawat = ? "
                          + "GROUP BY hasil_pemeriksaan_usg_gambar.photo"
                      );
                      ...
                      while (rs.next()) {
                          try {                                         
                              ...
                              String template = Sequel.cariIsi("SELECT wa_template FROM wa_template WHERE no_template=?", "USG");
                              String countryCode = "62";
                              String pesan = template.replace("{nama}", rs.getString("nm_pasien"))
                                                    .replace("{rm}", rs.getString("no_rkm_medis"))
                                                    .replace("{antrian}", "")
                                                    .replace("{poli}", "")
                                                    .replace("{diagnosa}", tbObat.getValueAt(tbObat.getSelectedRow(), 8).toString())
                                                    .replace("{perujuk}", tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString())
                                                    .replace("{dokter}", tbObat.getValueAt(tbObat.getSelectedRow(), 5).toString())
                                                    .replace("{tanggal}", tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString());
                              String simpanPesan = pesan.replace("*", "").replace("_", "").replace("{", "").replace("}", "").replace("|", "");
                              String urlHasil = koneksiDB.URLWEBAPPS() + "hasilpemeriksaanusg/" + rs.getString("photo");
                              ...
                              try (BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream()))) {
                                  ...
                                  if ("false".equals(jsonNode.get("status").asText())) {
                                      ...
                                      Sequel.queryu("INSERT INTO wa_report VALUES ('" + jsonNode.get("id").get(0).asText() + "', '" + TNoRM.getText() + "', '" + noWa + "', '" + akses.getkode() + "', 'Kirim Hasil USG', '" + jsonNode.get("target").get(0).asText() + "', '" + simpanPesan + "', '', '" + jsonNode.get("status").asText() + "', '', '" + dateNow + "', '" + dateNow + "')");
